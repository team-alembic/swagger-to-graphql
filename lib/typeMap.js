...
var isObjectType = function isObjectType(jsonSchema) {
  return jsonSchema.properties || jsonSchema.$ref || jsonSchema.type === "object" || jsonSchema.type === "array" || jsonSchema.schema;
};

var getTypeNameFromRef = function getTypeNameFromRef(ref, isInputType) {
  var baseTypeName = ref.replace("#/definitions/", "").replace(/\//, "_");
  // return isInputType ? baseTypeName + "Input" : baseTypeName;
  console.log({ ref: ref, baseTypeName: baseTypeName });
  return isInputType ? baseTypeName : baseTypeName;
};

var getExistingType = function getExistingType(ref, isInputType) {
  var baseTypeName = getTypeNameFromRef(ref, isInputType);
  var typeName = baseTypeName + (isInputType ? "Input" : "");
  ...
  if (!__allTypes[typeName]) {
    var schema = allSchema.definitions[baseTypeName];
    console.log({ typeName: typeName, schema: schema });
    if (!schema) {
...
var createGQLObject = exports.createGQLObject = function createGQLObject(jsonSchema, title, isInputType) {
  ...
  console.log({ reference: reference, jsonSchema: jsonSchema });
  if (reference) {
...
var jsonSchemaTypeToGraphQL = function jsonSchemaTypeToGraphQL(title, jsonSchema, schemaName, isInputType) {
  if (isObjectType(jsonSchema)) {
    if (jsonSchema.in == "body" && jsonSchema.name == "body") {
      return jsonSchemaTypeToGraphQL(title + "_" + schemaName, jsonSchema.schema, schemaName, isInputType);
    } else {
...
var mapParametersToFields = exports.mapParametersToFields = function mapParametersToFields(parameters, typeName) {
  ...
  return parameters.reduce(function (res, param) {
    var type = jsonSchemaTypeToGraphQL(typeName, param.jsonSchema, param.name, true);
    res[param.name] = {
      type: type
    };
    return res;
